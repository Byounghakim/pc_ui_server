...
const AutomationProcess: React.FC<AutomationProcessProps> = ({ 
  ...
  useEffect(() => {
    ...
    localStorage.setItem('automation-client-id', clientId);
    ...
    try {
      ...
      if (
        ...
        const automationState = {
          ...
          waitingCountdowns,
          lastUpdated: Date.now(),
          clientId: `client_${Math.random().toString(36).substring(2, 9)}` // 클라이언트 식별자
        };
  ...
  // resetAutomation 함수 복원 - 리셋 우선처리 로직 추가
  const resetAutomation = () => {
  ...
  // fullReset 함수 복원 - 완전 초기화 우선처리 로직 추가
  const fullReset = () => {
    console.log('자동화 공정 완전 초기화 시작');
    ...
    try {
      ...
      if (topic === EXTRACTION_INPUT_TOPIC) {
        ...
        try {
          ...
          // 본인이 발행한 메시지는 무시
          if (automationState.clientId === localStorage.getItem('automation-client-id')) {
            return;
          }
          ...
          <div className="p-2 space-y-2 max-h-[400px] overflow-y-auto">
            {selectedSequences.map((item, index) => (
              ...
              >
                <div className="flex-1">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      {item.status === 'waiting' && currentSequenceIndex === index - 1 && status === 'running' && (
                        <div className="text-xs text-blue-600 animate-pulse">
                          {waitingCountdowns[index] > 0 
                  ...
                  <div className="text-xs text-gray-500 mt-1 flex items-center gap-3">
                    <span>모드: {item.sequence.operation_mode}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center">
                    ...
                    <button
                      ...
                      disabled={false} // 항상 활성화
                      className="w-[80px] h-8 px-2 border rounded text-sm flex items-center justify-between bg-white"
                    >
                ...
                {/* 대기 시간 팝업 */}
                {showTimePopup === index && (
                  <div className="absolute z-10 bg-white border shadow-lg rounded-md p-3 mt-1" style={{ 
                    right: 'auto', 
                    top: 'auto', 
                    position: 'fixed',
                    ...
                    <div className="space-y-3">
                      <h3 className="font-medium text-sm">대기 시간 설정</h3>
                      
                      <div className="flex gap-2 items-center">
                        <div>
                          <span className="text-xs block mb-1">시간</span>
                          <Select
                            value={tempHours.toString()}
                            onValueChange={(value) => setTempHours(parseInt(value))}
                          >
                        ...
                        <div>
                          ...
                          <Select
                            value={tempMinutes.toString()}
                            onValueChange={(value) => setTempMinutes(parseInt(value))}
                          >
                            <SelectTrigger className="w-[80px] h-8">
                              <SelectValue placeholder="0" />
                            </SelectTrigger>
                        ...
                        <div>
                          ...
                          <Select
                            value={tempSeconds.toString()}
                            onValueChange={(value) => setTempSeconds(parseInt(value))}
                          >
                            <SelectTrigger className="w-[80px] h-8">
                              <SelectValue placeholder="0" />
                            </SelectTrigger>
                            <SelectContent>
                              {Array.from({ length: 60 }, (_, i) => (
                                <SelectItem key={`second-${i}`} value={i.toString()}>
                                  {i}초
                                </SelectItem>
                              ))}
                            </SelectContent>
