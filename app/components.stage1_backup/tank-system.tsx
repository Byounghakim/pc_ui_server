      ...
      try {
        ...
        if (topic === 'extwork/extraction/input') {
          ...
          try {
            ...
            // progress 메시지 업데이트 - rawJson 필드에 원본 JSON 데이터 저장
        ...
        // 토픽에 따른 처리
          ...
          if (tankLevelMatch) {
            ...
            // 탱크 가득참 경고 메시지 처리
            if ((messageStr.includes("가득") || messageStr.includes("full") || messageStr.includes("100%")) && isPumpRunning) {
              // 펌프가 켜져 있고 탱크가 가득 찬 경우 경고 알림 추가
              const warningMessage = `주의: 탱크 ${tankId}가 가득 찼습니다. 펌프 ${inverterId}가 아직 작동 중입니다.`;
              addNotification(warningMessage, 'warning', inverterId);
            }
            ...
            setTankMessages(prev => {
              ...
              // 부모 컴포넌트의 tankData를 업데이트하기 위해 
              // 로컬 스토리지에 저장합니다. 부모 컴포넌트에서
              // 다음 렌더링 시 이 메시지를 읽을 수 있습니다.
        ...
        if (!isNaN(remaining) && !isNaN(elapsed) && total > 0) {
          ...
          try {
            if (latestProgress.rawJson) {
              ...
              if (jsonData.elapsedTime !== undefined && jsonData.totalTime !== undefined) {
  ...
  const getFillingStyle = (status: string, tankId: number, operationTime?: number) => {
    ...
    if (pumpStatus === "ON") {
      ...
      if (operationTime && operationTime > 0) {
        ...
        const fillPercent = Math.min((elapsedTime / operationTime) * 100, 100);
      ...
      // 가동 시간이 없으면 기존 진행 상황 정보 활용
      if (progressMessages.length > 0) {
        try {
          const latestProgress = progressMessages[0];
          const latestMessage = latestProgress.message || '';
          
          // "남은: XXs | 경과: YYs" 형식 패턴 확인
          const remainingMatch = latestMessage.match(/남은:\s*(\d+)s/) || latestMessage.match(/남음:\s*(\d+)s/);
          const elapsedMatch = latestMessage.match(/경과:\s*(\d+)s/) || latestMessage.match(/진행:\s*(\d+)s/);
          ...
          if (remainingMatch && elapsedMatch) {
            ...
            if (!isNaN(remaining) && !isNaN(elapsed) && total > 0) {
              // 경과 시간 비율로 계산
              const fillPercent = Math.min((elapsed / total) * 100, 100);
      ...
      <g key={`tank-${tankId}`} id={`tank-${tankId}`}>
        ...
        {(isPumpOn || status === "filling") && (
          <rect
            ...
            style={getFillingStyle(status, tankId)}
          />
        )}
    ...
    <div className="relative w-full h-[950px] bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100">
      ...
      <div className="flex h-[calc(100%-32px)]">
        ...
        <div className="w-[55%] border-r border-gray-200 p-0 pl-1 pr-1">
          ...
          {/* 본탱크 상태 메시지 텍스트 */}
          <text x={mainTankPosition.x} y={mainTankPosition.y + 30} textAnchor="middle" className="text-[14px] fill-gray-700">
              ...
              <div className="space-y-2">
                ...
                {progressMessages.filter(msg => msg.rawJson).slice(0, 1).map((msg, idx) => {
                  try {
                    ...
                    return (
                      <div key={`process-info-${idx}`}>
                        {jsonData?.name && (
              <div className="mb-1">
                            <span className="font-semibold">공정 이름:</span>{" "}
                            <span>{jsonData.name}</span>
              </div>
                        )}
                        {jsonData?.mode && (
              <div className="mb-1">
                            <span className="font-semibold">운영 모드:</span>{" "}
                            <span>{jsonData.mode === "sequential" ? "순차 모드" : 
                                  jsonData.mode === "concurrent" ? "동시 모드" : 
                                  jsonData.mode === "mixed" ? "혼합 모드" : jsonData.mode}</span>
              </div>
                        )}
                        {jsonData?.repeat !== undefined && (
                          <div className="mb-1">
                            <span className="font-semibold">반복 횟수:</span>{" "}
                            <span>{jsonData.repeat}회</span>
                      </div>
                        )}
                        {jsonData?.sequences && (
                          <div className="mb-1">
                            <span className="font-semibold">활성화될 펌프:</span>{" "}
                            <span>
                              {jsonData.sequences.map((seq: any) => 
                                seq.pump_id || seq.pumpId
                              ).filter(Boolean).join(', ')}
                            </span>
                  </div>
                        )}
                        {jsonData?.estimated_completion_time && (
                          <div className="mb-1">
                            <span className="font-semibold">예상 종료 시간:</span>{" "}
                            <span>{jsonData.estimated_completion_time}</span>
                </div>
                        )}
              </div>
                    );
                    ...
                    return (
                      <div key={`process-info-error-${idx}`} className="text-red-500">
                ...
                {progressMessages.filter(msg => msg.rawJson).length === 0 && (
                  <div className="text-gray-500 italic text-center p-2">
                    공정 명령이 없습니다. extwork/extraction/input 토픽으로 JSON 명령을 보내주세요.
                  </div>
                )}
              </div>
            </div>
