...
import { X, Play, Square, RotateCcw, ArrowUp, ArrowDown, PlusCircle, Pause, RefreshCw, Copy } from 'lucide-react';
...
const onMqttMessageArrived = (topic: string, message: Buffer) => {
  ...
  // TimePickerDial 컴포넌트를 상속받는 형태로 변경
  ...
  const TimePickerInline: React.FC<TimePickerInlineProps> = ({ 
    ...
    const { h, m, s } = useMemo(() => convertValueToTime(value), [convertValueToTime, value]);
    ...
    const handleTimeChange = useCallback((type: 'h' | 'm' | 's', newValue: number) => {
      let updatedHours = type === 'h' ? newValue : hours;
      let updatedMinutes = type === 'm' ? newValue : minutes;
      let updatedSeconds = type === 's' ? newValue : seconds;
      
      // 값 범위 확인
      updatedHours = Math.max(0, Math.min(23, updatedHours));
      updatedMinutes = Math.max(0, Math.min(59, updatedMinutes));
      updatedSeconds = Math.max(0, Math.min(59, updatedSeconds));
      ...
      // 변경사항 즉시 적용
      const totalSeconds = updatedHours * 3600 + updatedMinutes * 60 + updatedSeconds;
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center">
                    ...
                    <TimePickerInline
                      ...
                      onChange={(value) => handleWaitTimeChange(index, value)}
                      max={3600}
                      step={5}
                    />
                  </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <Button 
                            ...
                            onClick={() => {
                              // 현재 값에 5초 추가
                              const newSeconds = tempSeconds + 5;
                              // 초가 60을 넘어가면 분 증가
                              const additionalMinutes = Math.floor(newSeconds / 60);
                              const updatedSeconds = newSeconds % 60;
                              const newMinutes = tempMinutes + additionalMinutes;
                              // 분이 60을 넘어가면 시간 증가
                              const additionalHours = Math.floor(newMinutes / 60);
                              const updatedMinutes = newMinutes % 60;
                              const newHours = tempHours + additionalHours;
                              
                              setTempSeconds(updatedSeconds);
                              setTempMinutes(updatedMinutes);
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              // 현재 값에 30초 추가
                              const newSeconds = tempSeconds + 30;
                              // 초가 60을 넘어가면 분 증가
                              const additionalMinutes = Math.floor(newSeconds / 60);
                              const updatedSeconds = newSeconds % 60;
                              const newMinutes = tempMinutes + additionalMinutes;
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              // 현재 값에 1분 추가
                              const newMinutes = tempMinutes + 1;
                              // 분이 60을 넘어가면 시간 증가
                              const additionalHours = Math.floor(newMinutes / 60);
                              const updatedMinutes = newMinutes % 60;
                              const newHours = tempHours + additionalHours;
